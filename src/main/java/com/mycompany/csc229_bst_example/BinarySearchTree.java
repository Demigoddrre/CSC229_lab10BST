...
private void doPreOrder(BstNode root) {
    ...
    Integer leftHeight = doFindHeight(root.getLeft());
    Integer rightHeight = doFindHeight(root.getRight());
    return Math.max(leftHeight, rightHeight) + 1;
}

    public int getDepth(BstNode node) {
        //ToDo 4: complete getDepth of a node 
          return doGetDepth(this.root, node, 0);
    }
    
    
    private int doGetDepth(BstNode root, BstNode node, int depth) {
    if (root == null) {
        return -1;
    }
    if (root == node) {
    ...
    int leftDepth = doGetDepth(root.getLeft(), node, depth + 1);
    if (leftDepth != -1) {
        return leftDepth;
    }
    return doGetDepth(root.getRight(), node, depth + 1);
}
   public void print() {
       System.out.println("\n==== BST Print ===== \n");
        print("", root, false);
